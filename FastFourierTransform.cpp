...
namespace bear::dsp
{
    ...
    void FastFourierTransformBase::forward(const vector<float>& input, vector<complex<float>>& output)
    {
        ...
        interleave(const vector<float>&(real), const vector<float>&(imaginary), vector<float>&((float*)output.data(), output.size() * 2));
    }
    ...
    void FastFourierTransformBase::forward(const vector<double>& input, vector<complex<double>>& output)
    {
        ...
        interleave(const vector<double>&(real), const vector<double>&(imaginary), vector<double>&((double*)output.data(), output.size() * 2));
    }
    ...
    void FastFourierTransformBase::inverse(const vector<complex<float>>& input, vector<float>& output)
    {
        ...
        // Deinterleave
        deinterleave(const vector<float>&((float*)input.data(), input.size() * 2), vector<float>&(real), vector<float>&(imaginary));
    ...
    void FastFourierTransformBase::inverse(const vector<complex<double>>& input, vector<double>& output)
    {
        ...
        // Deinterleave
        deinterleave(const vector<double>&((double*)input.data(), input.size() * 2), vector<double>&(real), vector<double>&(imaginary));
    ...
    void FastFourierTransformBase::forwardComplex(const vector<complex<float>>& input, vector<complex<float>>& output)
    {
        ...
        // Deinterleave
        deinterleave(const vector<float>&((float*)input.data(), input.size() * 2), vector<float>&(inReal), vector<float>&(inImaginary));
        ...
        interleave(const vector<float>&(outReal), const vector<float>&(outImaginary), vector<float>&((float*)output.data(), output.size() * 2));
    }
    ...
    void FastFourierTransformBase::forwardComplex(const vector<complex<double>>& input, vector<complex<double>>& output)
    {
        ...
        // Deinterleave
        deinterleave(const vector<double>&((double*)input.data(), input.size() * 2), vector<double>&(inReal), vector<double>&(inImaginary));
        ...
        interleave(const vector<double>&(outReal), const vector<double>&(outImaginary), vector<double>&((double*)output.data(), output.size() * 2));
    }
    ...
    void FastFourierTransformBase::inverseComplex(const vector<complex<float>>& input, vector<complex<float>>& output)
    {
        ...
        // Deinterleave
        deinterleave(const vector<float>&((float*)input.data(), input.size() * 2), vector<float>&(inputReal), vector<float>&(inputImaginary));
        ...
        interleave(const vector<float>&(outputReal), const vector<float>&(outputImaginary), vector<float>&((float*)output.data(), output.size() * 2));
    }
